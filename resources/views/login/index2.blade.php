            ...
            <div class="row">
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 border-0">
                    <div class="border-0">
                        ...
                        @endif
                        <div class="card-body border-0 mb-2">
                            <h3>Select User Account to Login</h3>
                            <form action="{{route('login_api')}}" method="POST">
                            {{-- <form action="/login-apis" method="POST"> --}}
                                @csrf

                                <select name="account" id="" class="custom-select custom-select-lg mt-1 autofocus required">
                                    <option value="homis">Login using HOMIS Account</option>
                                    <option value="cdoe">Login using CDOE Account</option>
                                </select>
                                <hr>
                                <label for="user_name">Username</label>
                                <input id="user_name" type="text" class="form-control" name="username" value="{{ old('user_name') }}" placeholder="Username" required autofocus>
                                <br>
                                <label for="user_pass">Password</label>
                                <input id="user_pass" type="password" class="form-control{{ $errors->has('user_pass') ? ' is-invalid' : '' }}" placeholder="Password" name="password" required>
                                <button type="submit" class="btn btn-primary err sub mt-3">Login</button>
                            </form>
                        </div>
                        <div class="card-footer text-center bg-white">
                            <small class="float-right text-muted">Developed: Dexter Balcita - IHOMS | Local 202</small>
                        </div>
                    </div>
                </div>
            </div>
