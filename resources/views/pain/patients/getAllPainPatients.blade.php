                    ...
                    <table class="table table-condensed table-hover table-striped" id="myTable1">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                @if($pat->scheduled == 0
                                     || $pat->scheduled == null  )
                                    <span class="badge badge-danger">{{$pat->scheduled}}</span>
                                    
                                    @else
                                    <span class="badge badge-primary"> {{$pat->scheduled}}</span>
                                    @endif
                                ...
                                <td>
                                    ...
                                    <div class="row">
                                        ...
                                        <form action="{{ route('painScheduler', ['patientNoraEnccode' => $pat->enccode]) }}" method="POST">
                                            @csrf
                                            <input name="patientPainEnccode" type="hidden" value="patientPainEnccode" hidden>
                                            <input type="text" value="{{$pat->enccode}}" name="enccode" hidden>
                                            <input name="patientPainHpercode" type="hidden" value="patientPainHpercode" hidden>
                                            <input type="text" value="{{$pat->hpercode}}" name="patientPainHpercode" hidden>
                                    ...
                                    @if(App\Http\Controllers\LoggedUser::getuser())
                                    <form action="{{ route('painScheduler', ['patientPainEnccode' => $pat->enccode]) }}" method="POST">
                                        @csrf
                                        <input name="patientPainEnccode" type="hidden" value="patientPainEnccode" hidden>
                                        <input type="text" value="{{$pat->enccode}}" name="enccode" hidden>
                                        <input name="patientPainHpercode" type="hidden" value="patientPainHpercode" hidden>
                ...
                if (data.length > 0) {
                    ...
                    data.forEach(element => {
                        template += '<tr>' +
                            ...
                            '<td>' +
                            '<form action="/noraHome" method="POST" enctype="multipart/form-data">' +
                            '@csrf' +
                ...
                if (data.length > 0) {
                    ...
                    data.forEach(element => {
                        template += '<tr>' +
                            ...
                            '<td>' + element.findx + '</td>' +
                            '<td>' +
                            '<form action="/painPatientdetails" method="POST" enctype="multipart/form-data">' +
                            '@csrf' +
