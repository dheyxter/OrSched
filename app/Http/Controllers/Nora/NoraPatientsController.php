...
class NoraPatientsController extends Controller
{
    ...
    public function noraPatdetails(request $request)
    {
        // $patientdetails = patient::where('id', $request->patient_id)->with('reservation')->first();
        $noraPatientdetails = toAccept::where('id', $request->patient_id)->with('reservation')
        ->join('hospital.dbo.hpersonal', 'jhay.vw_toAccept.entry_by', '=', 'dbo.hpersonal.employeeid')
        ->first();
        return response()->json($noraPatientdetails);
    }

    public function noraPatientdetails(request $request)
    {        
        ...
        $getPat = DB::UPDATE("EXEC nora.paul.nora_getPatients '$request->enccode',  '$request->hpercode', '".Auth::user()->employeeid."'");
    ...
    public function Nora_JS_GenPatientList(request $request)
    {
